...
public class Player : MonoBehaviour
{
    ...
    {
        Debug.Log("OnCollisionEnter2D"); // Collider 와 부딪쳤을때 실행
    }

    private void OnCollisionStay2D(Collision2D collision)
    {
        Debug.Log("OnCollisionEnter2D"); // Collider 와 계속 접촉하고 있을 때 (매 프레임마다 호출)
    }

    private void OnCollisionExit2D(Collision2D collision)
    {
        Debug.Log("OnCollisionEnter2D"); // Collider 와 접촉이 떨어지는 순간 실행
    }

    private void OnTriggerEnter2D(Collider2D collision)
    {
        Debug.Log("OnCollisionEnter2D"); // Trigger 에 들어갔을때 실행... collider 랑 다른점은 trigger 는 겹치기 가능
    }

    private void OnTriggerStay2D(Collider2D collision)
    { 
        Debug.Log("OnCollisionEnter2D"); // Trigger 와 겹쳐있을때 (매 프레임마다 호출)
    }

    private void OnTriggerExit2D(Collider2D collision)
    {
        Debug.Log("OnCollisionEnter2D"); // Trigger 와 나갔을 때
    }
    ...
    private void OnFire(InputAction.CallbackContext context)
    {
        ...
        //ㅊfloat value = Random.Range(0.0f, 10.0f); //using system 을 없애야함!! .. value 에는 0.0 ~ 10.0 의 랜덤값이 들어간다.
